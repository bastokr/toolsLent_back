...
import 'package:escroomtok/actions/videourl.dart';
import 'package:escroomtok/actions/votecolorhelper.dart';
import 'package:escroomtok/widgets/videoplayeritem.dart';
import 'package:escroomtok/generated/i18n.dart';
import 'package:escroomtok/models/credits_model.dart';
...
Widget buildView(
    BangtalThemaPageState state, Dispatch dispatch, ViewService viewService) {
  ...
  Widget _buildCreditsCell(CastData p) {
    return GestureDetector(
      onTap: () => dispatch(BangtalThemaPageActionCreator.onCastCellTapped(
                  ...
                  decoration: BoxDecoration(
                      ...
                      image: DecorationImage(
                          ...
                          image: CachedNetworkImageProvider(
                              state.document['posterPath'] == null
                                  ? ImageUrlDefault.emptyimage
                                  : ImageUrlDefault.getUrl(
                                      state.document['posterPath'],
                                      ImageSize.w300)))),
                ),
              ),
              Padding(
  ...
  Widget _buildRecommendationCell(VideoListResult d) {
    return GestureDetector(
      ...
      onTap: () => dispatch(
          BangtalThemaPageActionCreator.onRecommendationTapped(
              d.id, d.backdropPath)),
    );
  }
  ...
  Widget _buildImageCell(ImageData d) {
    ...
    return Container(
      ...
      child: ParallaxImage(
          extent: h,
          image: CachedNetworkImageProvider(ImageUrlDefault.getUrl(
              state.document['posterPath'], ImageSize.w300))),
    );
  }
  ...
  Widget _getPosterPic() {
    if (state.document['posterPath'] == null)
      return SizedBox(
          child: Shimmer.fromColors(
  ...
  Widget _getTitle() {
    print(state.document['thema']);
    if (state.document['thema'] == null)
      return SizedBox(
          child: Shimmer.fromColors(
        baseColor: Colors.grey[500],
        highlightColor: Colors.grey[100],
  ...
  Widget _getOverWatch() {
    if (state.document['conts'] == null)
      ...
      return Text(state.document['conts'],
          style: TextStyle(
              color: Colors.black,
              fontSize: Adapt.px(30),
              height: 1.2,
                ...
                image: DecorationImage(
                    ...
                    image: CachedNetworkImageProvider(
                        state.document['posterPath'] == null
                            ? ImageUrlDefault.emptyimage
                            : ImageUrlDefault.getUrl(
                                state.document['posterPath'], ImageSize.w500)),
                    fit: BoxFit.cover)),
          ),
          Container(
            width: Adapt.screenW(),
              ...
              children: <Widget>[
                ...
                Container(
                  child: Row(
                    children: <Widget>[
                      Text(
                          'Level:' + state.document['level_p'].toString() ??
                              '0',
                          style: TextStyle(
                              fontWeight: FontWeight.w500,
                              fontSize: Adapt.px(30),
                ...
                Container(
                  child: Row(
                    children: <Widget>[
                      Text(
                          'Person: ' + state.document['person'].toString() ??
                              '0',
                          style: TextStyle(
                              fontWeight: FontWeight.w500,
                              fontSize: Adapt.px(30),
  ...
  return Scaffold(
    ...
    floatingActionButton: FloatingActionButton.extended(
      onPressed: () async {
        ...
        if (await canLaunch(state.document['targetUrl'])) {
          await launch(state.document['targetUrl']);
        } else {
          Toast.show('결재페이지가 연동되지않았습니다.', state.scaffoldkey.currentContext);
        }
      },
      label: Text('결재페이지'),
                  ...
                  sliver: SliverAppBar(
                      ...
                      actions: <Widget>[
                        ...
                        IconButton(
                          ...
                          onPressed: () => dispatch(
                              BangtalThemaPageActionCreator.openMenu()),
                        )
                        */
                      ],
